...
- 捲軸同時是整份日誌的地圖：警告的密度、錯誤與書籤都標在捲軸上，點標記就跳過去。在捲軸上按右鍵可選擇要顯示哪幾種標記。
...
- 介面支援 7 種語言（English、繁體中文、简体中文、日本語、한국어、Русский、Español），預設跟隨系統語言。新增語言只需一個檔案的 PR——見 `src/locales/en.ts`。

## 效能

解析在原生的 Rust 核心、背景執行緒上進行，所以載入檔案時視窗不會卡住。清單採用虛擬化、分頁按需載入，前端不會持有整份日誌。即使日誌到了數 GB，捲動和篩選也還是順的；而且不論條目再多都到得了：清單以「列」而非像素為單位捲動，不受瀏覽器引擎單一元素高度上限的限制（那個上限會讓大約一百萬列之後的內容永遠捲不到）。
